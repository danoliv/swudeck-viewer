...
- Never state something is fixed, deployed, or visible without first checking — don't infer from memory of what you intended to do. Before claiming a UI change is visible: confirm which tab/URL is actually open (`list_pages`/`select_page`), whether it's localhost vs. the deployed site, and whether the relevant commits were actually pushed and deployed (check `git status`/`git log` and the GitHub Actions run, not just "I made the edit").
- If the user states something as fact (e.g. "X is broken", "the fix isn't there", "you didn't do Y"), verify it against the actual current state (files, running server, live page, git history) before responding — don't answer from assumption or take the claim at face value either. Report what you actually observed.
- For small/ambiguous visual fixes (spacing, alignment, sizing), don't iterate blindly through multiple CSS guesses while re-measuring DOM rects over and over. If the request is underspecified (e.g. "a little spacing" vs. literally aligning elements are different, sometimes mutually exclusive, fixes) or your first attempt visibly misses the mark, stop and ask the user what the end state should look like rather than guessing again. Burning many tool calls on a one-line CSS tweak is itself a sign to stop and ask.
